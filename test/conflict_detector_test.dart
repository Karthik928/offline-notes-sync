...
void main() {
  group('ConflictDetector', () {
    ...
    test('detects a conflict when local and server both changed after the last sync', () {
      ...
      expect(detector.hasConflict(localNote, serverRecord, DateTime(2024, 1, 3)), isTrue);
    });

    test('does not report a conflict when the server matches the local snapshot', () {
      final localNote = Note(
        id: 'local-2',
        title: 'Same title',
        body: 'Same body',
      ...
      expect(detector.hasConflict(localNote, serverRecord, DateTime(2024, 1, 3)), isFalse);
    });
  });
}
